...
export const router = createRouter({
   ...
   */
  routes: [...notFoundAndNoPower, ...staticRoutes, ...formRoutes, ...projectRoutes, ...clientRoutes]
});
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.query && to.query.accessToken) {
    ...
    next(`/tokenLogin?thirdToken=${to.query.accessToken}&redirect=${to.path}&params=${JSON.stringify(params)}`);
    return;
  }
  ...
  if (to.path === "/login" && !token) {
    ...
    if (to.meta && to.meta.requireLogin == false) {
      next();
      NextLoading.done();
    } else if (!token) {
      toLogin();
