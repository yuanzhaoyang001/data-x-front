...
const setSendChildren = (path: string) => {
  // 移除path的前缀
  // const currentPathSplit = path.replace(adminPrefix, "").split("/");
  let currentData: MittMenu = { children: [] };
  // TS_UPDATE 修改了next-admin的代码 不然不显示
  state.columnsAsideList.map((v: RouteItem, k: number) => {
    // 等于或者前面的两级路由相等 表示是子菜单
    if (v.path === path || isMatchPath(path, v.path)) {
      v["k"] = k;
      currentData["item"] = { ...v };
 ...
 * 路径是否匹配
 * @param path 路径
 * @param matchPath 待匹配的路径
 */
const isMatchPath = (path: string, matchPath: string) => {
  const currentPathSplit = path.split("/");
  if (path.startsWith(adminPrefix) && matchPath.startsWith(adminPrefix)) {
    return matchPath === `/admin/${currentPathSplit[2]}`;
  } else {
    return matchPath === `/${currentPathSplit[1]}`;
  }
};
