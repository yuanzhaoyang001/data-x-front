 ...
 * @param formKey 表单Id
 */
export function saveWriteCount(formKey: string) {
  try {
 ...
 *
 */
export function checkWriteCountCap(formKey: string, totalCount: number) {
  const count = getWriteCount(formKey);
 ...
 * 保存
 */
function setWriteCount(formKey: string, writeCount: number = 1) {
 ...
 */
function getWriteCount(formKey: string) {
