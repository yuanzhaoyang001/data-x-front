...
function handleGenTable(row) {
  ...
  if (row.genType === "1") {
    genCode(row.tableName).then(response => {
      proxy.msgSuccess("成功生成到自定义路径：" + row.genPath);
    });
  } else {
    downLoadZip("/tool/gen/batchGenCode?tables=" + tbNames, "tduck");
  }
}
...
function handleEditTable(row) {
  ...
  router.push({ path: "/gen/edit/" + id, query: { current: queryParams.value.current } });
}
