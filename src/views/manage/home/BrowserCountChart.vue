...
import { getFormSourceCountReq } from "@/api/mannage/analysis";
...
onMounted(async () => {
  // 加载数据
  try {
    const res = await getFormSourceCountReq();
    option.legend.data = res.data.map(item => item.name || i18n.global.t("system.home.unknown"));
    option.series[0].data = res.data.map(item => {
      return {
        name: item.name || i18n.global.t("system.home.unknown"),
        value: item.count || 0
      };
    });
...
const option = {
  ...
  title: {
    text: i18n.global.t("system.home.browser"),
    x: "left",
