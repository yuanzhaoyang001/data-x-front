...
onMounted(async () => {
  ...
  try {
    ...
    const pc = ["Windows", "Mac OS", "Linux"];
    const data = [0, 0, 0];
    option.legend.data = [i18n.global.t("system.home.mobile"), "PC", i18n.global.t("system.home.other")];
    res.data.forEach(item => {
      if (mobiles.includes(item.name)) {
        data[0] += item.count;
      } else if (pc.includes(item.name)) {
        data[1] += item.count;
      } else {
        data[2] += item.count;
      }
    ...
    option.series[0].data = [
      {
        name: i18n.global.t("system.home.mobile"),
        value: data[0]
      },
      {
        name: "PC",
      ...
      {
        name: i18n.global.t("system.home.other"),
        value: data[2]
      }
    ];
  } catch (e) {
...
const colorList = ["#51A3FC", "#36C78B", "#FEC279", "#968AF5", "#E790E8", "#f9e264", "#f47a75", "#009db2"];

const option = {
  backgroundColor: state.charts.bgColor,
  title: {
    text: i18n.global.t("system.home.client"),
    x: "left",
