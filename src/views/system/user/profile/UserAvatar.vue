<template>
  ...
  >
    <img
      :src="options.img"
      :title="$t('system.userAvatar.title')"
...
const userStore = stores.userInfos;
...
function beforeUpload(file) {
  if (file.type.indexOf("image/") == -1) {
    proxy.msgError(i18n.global.t("system.userAvatar.error"));
  } else {
...
function uploadImg() {
  proxy.$refs.cropper.getCropBlob(data => {
    ...
    uploadAvatar(formData).then(response => {
      ...
      stores.updateAvatar(response.data.imgUrl);
      proxy.msgSuccess(i18n.global.t("formI18n.all.success"));
      visible.value = false;
      emits("uploadSuccess", response.data.imgUrl);
    });
  });
}
